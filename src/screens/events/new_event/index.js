...
import { map, get, set, mapValues, isArray, keys, sortBy } from 'lodash'
...
class Event extends Component {
  ...
  static save(props, values){
    ...
    action == "add" ? msg = 'Event Created' : msg = "Event Edited"
    const popup = {
  ...
  render() {
    ...
    return (
      <div className={classes.contaier}>
        ...
        <MainLayout
          active={activeTab} form={form} values={values}>
        </MainLayout>
      </div>
    )
  }
...
const mapStateToProps = (state, props) => ({
  initialValues: get(state, 'form.event.values', props.initialValues) || { basic: { _type: 'ev' } },
  order : state.orders__main.active
})
export const EventAdd = withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslation()(Form(Event))))
export class EditEvent extends Component {
  ...
  render() {
    return <EventAdd initialValues={this.initialValues} />
  }
}
