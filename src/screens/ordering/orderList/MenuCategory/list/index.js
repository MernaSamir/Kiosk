...
class RenderList extends Component {
    ...
    getComponent(props) {
        ...
        if (menu) {
            ...
            if (get(settings, 'menuShow', 'true') == "false") {
                return <SubCategory menu={menu} types={this.types} {...props} option={option} />
            }
            else {
    ...
    rendering = () => {
        const { match, setMain } = this.props
        const ListComponent = this.getComponent({ reset: this.resetSelect })
        if (match.url.includes('favorite')) {
            return <Favorite setMain={setMain} />
        }
        if (match.url.includes('search')) {
            return <Search setMain={setMain} />
        }
