...
export const build_childs = (params, data, state, props)=>{
    const totals = mergeWith({}, ...data, mergin.bind(this, props))
    ...
    return {
        ...
        childs: mapValues(groupBy(data, isArray(f_d.key) ? `key[${key}]`:`key`), (d, v)=>({
            id: v,
            ...build_childs(params, d, state, {...props, key: key+1})
        }))
    }
}
...
const getDaysRevenu = (props, bd_data, state, data, key)=>{
    ...
    let out = {
        ...
        get avg(){return round(this.net_sales / this.guests, 2)}
    }
    return out
}
