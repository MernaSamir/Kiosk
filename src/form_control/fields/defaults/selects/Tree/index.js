...
class SelectboxClass extends Component {
    ...
    renderOptions = () => {
        const { show = { key: 'GetDataSelector', show: 'name' }, tree, t, filteredData = {}, filteredChildData, selectParent = true } = this.props
    ...
    renderSubOptions = (aboveItem, child, filteredMainList, filteredData = {}) => {
        ...
        if (!isUndefined(child)) {
            ...
            if (child.options) {
                ...
                layerData = filteredData[child.reduxName] ?
                    values(filteredData[child.reduxName]) : applyFilter({
    ...
    render() {
        const { onSelectChange = this.handleChange, t, label, topLabel, placeholder = "Please select", field = {}, showParentVal, expand_all = false, multiple = true, validates = {} } = this.props
            ...
            <BasicComponent compare={this.compares}>
                <div>
                    ...
                    <TreeSelect
                        ...
                        value={field.value}
                        defaultValue={field.value ? field.value : multiple ? [] : ''}
                        ...
                        treeDefaultExpandAll={expand_all}
...
class select_connector extends Component {
    ...
    filterChildData(child) {
        ...
        let mainList = filteredData[child.reduxName] ?
            values(filteredData[child.reduxName]) : applyFilter({ key: 'Filter', path: child.reduxName })
        return filter ? applyFilter(filter, mainList, undefined, mainValues) : mainList
