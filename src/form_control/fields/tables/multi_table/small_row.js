...
class TableCollapseRow extends InputComponent {
    ...
    renderArrow = () => {
        const { child, collapseAll } = this.props;
    ...
    renderTCell(f, is_multi, key, index, props={}) {
        ...
        if ((f.parent || included) && child) {
            ...
            if(!is_multi){
                ...
                totalProps.mainValues = get(props.values, `total.${data.id}`, '')
                totalProps.mainChange = totalChange
    ...
    renderGroupBody = () => {
        const { data, show, child, padIndex, extra_show, collapseAll } = this.props;
        const { expand } = this.state;
        return <>
            <tr>
                <TDetails
