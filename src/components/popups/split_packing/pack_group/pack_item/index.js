...
class PackItem extends Component {
    ...
    serve = () => {
        ...
        setMain('orders__details', {
            item: {
                action: "update", id: orderDetail.id, status: 'served',
                serve_time: new Date(),
...
const mapStateToProps = (state, props) => ({
    orderDetail: get(state.orders__details.data, props.pack_id, {}),
    prices: state.items__prices.data,
    items: state.items__sales_items.data,
    get _price() {return get(state.items__prices.data, this.orderDetail.item)},
